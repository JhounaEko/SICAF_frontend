...
const HeaderNavbar = ({ functionRefreschDataTable, searchData, searchDataParameter, searchDataYear }) => {
    ...
    const [placeHolderInputSearch, setPlaceHolderInputSearch] = useState("descripción o abreviación");
    ...
    const dataYear = useRef('');
    const changeParameterYear = () => {
        const today = moment();
        console.log(dataYear.current);
        if (dataYear.current == "") {
            searchDataYear("")
        } else {
            if (dataYear.current > 1900 && dataYear.current <= today.year()) {
                searchDataYear(dataYear.current)
            } else {
                addNotification('warning', 'Aviso', 'El año debe estar entre 1900 y el año actual.', 'top-right', 8000, "fas fa-exclamation-circle", null)
            }
        }
    ...
    return (<>
        ...
        <div className="row">
            <div className="col-lg-2">
                <button className="btn btn-theme btn-rounded px-4 rounded-pill" title = "Agregar registro" onClick={openModal}><i className="fas fa-plus fa-1_5x"></i>fuente finan..</button>
            </div>
            ...
            <div className="col-lg-3">
                <select
                    ...
                    <option value="search">descripción o abreviación</option>
                    <option value="number">código</option>
                    <option value="data_create">fecha de registro</option>
                </select>
            </div>
            <div className="col-lg-3">
                <div className="input-group" title = "Busqueda por gestion">
                    <input type="number"
                        className="form-control"
                        placeholder='Introduzca el año'
                        onChange={(e) => { dataYear.current = e.target.value; }} />
                    <button type="button" className="btn btn-theme" onClick={() => changeParameterYear()} >
