    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row) => <div className="m-0 p-0">
                ...
                <p className="m-0 p-0" style={{ fontSize: '12px' }}> {row.place.description}</p>
            </div>,
            width: '200px'
        },
        {
            name: (<p className="m-0" style={{ fontWeight: 'bold', fontSize: '13px', textDecoration: 'underline' }}>Cargo</p>),
            cell: (row) => (<p className="m-0 p-0" title={row.position.description} style={{ fontSize: '12px' }}>{row.position.name}</p>),
...
<div className="d-flex justify-content-end align-items-center mb-3 gap-2">
  ...
  <div>
    ...
    <ExcelExport
        titulo={titulo}
        columnas={["id", "full_name", "office.name", "position.name", "created_at", "updated_at", "state.name"]}
        data={getDataTables}
      />
  </div>
  {/* //// FIN MOD EXCEL //// */}
