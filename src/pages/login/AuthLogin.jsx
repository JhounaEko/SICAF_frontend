...
export const useInitSesion = () => {
	...
	const InitSesion = (data, checked, { changeOptionBoton, openModal, addDataUserRolesMenus }) => {
			...
			if (response.data.statusCode === 200) {
				...
				Swal.fire({
					...
					if (result.dismiss === Swal.DismissReason.timer) {
						...
						/** Verificamos si al menos tiene un rol asignado*/
						if ((dataUser.roles).length === 1) {
							Cookies.set(process.env.REACT_APP_COOKIES_NAME_DATA, JSON.stringify(dataUser), { expires: parseInt(process.env.REACT_APP_TIME_COOKIES, 10) });
							try {
								navigate(dataUser.roles[0].menus[0].route, { state: response.data.results });
							} catch (error) {
								console.log(error)
								Cookies.remove(process.env.REACT_APP_COOKIES_NAME_DATA);
								Cookies.remove(process.env.REACT_APP_COOKIES_NAME_DATA);
								Swal.fire({
									title: "El rol seleccionado no tiene menús asignados",
									icon: "warning",
									draggable: true,
									timer: 3000,
							...
							addDataUserRolesMenus(dataUser);
							openModal();
						}
					}
				});
