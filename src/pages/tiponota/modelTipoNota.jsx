    ...
    const useCreate = async (dataForm) => {
        ...
        try {
            const respose = await axios.post(
                process.env.REACT_APP_API_URL + '/api/v1/note_types',
                dataForm
                , {
                    headers: {
            ...
            try {
                if (error.response.status === 403) {
                    addNotification('info', 'Aviso', "No tiene permisos para registrar tipo de nota", 'top-right', 8000, "fas fa-exclamation-circle", null)
                } else {
...
export const modelUseListTable = () => {
    const useListTable = async (getPag = 1, getSortColumn = 'id', getOrder = "desc", getCountRows = 10, getSearh = "", getParameterSearh = "search", getYear = "") => {
        ...
        try {
            ...
            try {
                if (error.response.status === 403) {
                    addNotification('info', 'Aviso', "No tiene permisos para la listar los tipos de notas", 'top-right', 8000, "fas fa-exclamation-circle", null)
                } else {
                    if (error.code == "ERR_BAD_REQUEST") {
                        if (error.response.data.message === "Unauthenticated.") {
                            Cookies.remove(process.env.REACT_APP_COOKIES_NAME_TOKEN);
    ...
    const useChangeDataRow = async (dataForm) => {
        ...
        try {
            const respose = await axios.patch(
                `${process.env.REACT_APP_API_URL}/api/v1/note_types/${dataForm.id}`,
                dataForm,
                {
                    headers: {
    ...
    const useChangeStatus = async (statusRow, idRow) => {
        ...
        try {
            const response = await axios.patch(
                `${process.env.REACT_APP_API_URL}/api/v1/note_types/${idRow}`,
                {
