...
import { modelUseListTable, modelChangeStatus } from '../modelFuentesOrganizaciones.jsx';
...
const TableList = (getDataRefresch) => {
    ...
    const columns = [
        ...
        {
            name: (<p className="m-0" style={{ fontWeight: 'bold', fontSize: '15px', textDecoration: 'underline' }}>Código</p>),
            sortable: true,
            width: '100px',
            selectorKey: 'code',
            cell: (row) => <b>{row.code} </b>,
        },
        {
            name: (<p className="m-0" style={{ fontWeight: 'bold', fontSize: '15px', textDecoration: 'underline' }}>Descripción</p>),
            sortable: true,
            selectorKey: 'description',
            cell: (row) => ( <p>{row.description} <b>({row.abbreviation})</b> </p> ) ,
            width: '310px',
        },
        {
            name: (<p className="m-0 text-center" style={{ fontWeight: 'bold', fontSize: '13px', textDecoration: 'underline' }}>Año</p>),
            sortable: true,
            selectorKey: 'year',
            cell: (row) => <p style={{ fontSize: '14px' }} title=""> {row.year}  </p>,
            width: '110px',
        },
        {
