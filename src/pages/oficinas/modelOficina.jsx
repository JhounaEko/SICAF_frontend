...
export const modelUseListTable = () => {
    const useListTable = async (getPag = 1, getSortColumn = 'id', getOrder = "desc", getCountRows = 10, getSearh = "", getParameterSearh = "search", getLevel = "") => {
...
export const modelUseCreateLocationOffice = () => {
    const useCreateLocationOffice = async (idOffice, idPlaces) => {
        const sessionTokenSicaf = Cookies.get(process.env.REACT_APP_COOKIES_NAME_TOKEN);
        let decryptedToken;
        if (sessionTokenSicaf) {
            decryptedToken = CryptoJS.AES.decrypt(sessionTokenSicaf, process.env.REACT_APP_API_KEY).toString(CryptoJS.enc.Utf8);
        ...
        try {
            ...
            try {
                if (error.response.status === 403) {
                    addNotification('info', 'Aviso', "No tiene permisos para registrar localización de oficinas", 'top-right', 8000, "fas fa-exclamation-circle", null)
                } else {
                    if (error.code == "ERR_BAD_REQUEST") {
                        if (error.response.data.message === "Unauthenticated.") {
                            Cookies.remove(process.env.REACT_APP_COOKIES_NAME_TOKEN);
    ...
    return useCreateLocationOffice;
}

export const modelUseListTableOfficeLocation = () => {
    const useListTableOfficeLocation = async (idOffice) => {
        let returnResponse = {
            status: false,
            response: {},
            title: "",
        ...
        try {
            ...
            try {
                if (error.response.status === 403) {
                    addNotification('info', 'Aviso', "No tiene permisos para listar datos de ubicacion de oficina", 'top-right', 8000, "fas fa-exclamation-circle", null)
                } else {
                    if (error.code == "ERR_BAD_REQUEST") {
                        if (error.response.data.message === "Unauthenticated.") {
                            Cookies.remove(process.env.REACT_APP_COOKIES_NAME_TOKEN);
    ...
    return useListTableOfficeLocation;
}

export const modelChangeStatusOfficeLocation = () => {
    const useChangeStatusOfficeLocation = async (statusRow, idRow) => {
        const sessionTokenSicaf = Cookies.get(process.env.REACT_APP_COOKIES_NAME_TOKEN);
        let decryptedToken;
        if (sessionTokenSicaf) {
            decryptedToken = CryptoJS.AES.decrypt(sessionTokenSicaf, process.env.REACT_APP_API_KEY).toString(CryptoJS.enc.Utf8);
        ...
        try {
            const response = await axios.patch(
                `${process.env.REACT_APP_API_URL}/api/v1/office_locations/${idRow}`,
                {
                    "state_id": (statusRow === "INACTIVO") ? 1 : 2
                }, {
                headers: {
    ...
    return useChangeStatusOfficeLocation;
}
