...
const HeaderNavbar = ({
  ...
  const [parameterSearchDataCreate, setParameterSearchDataCreate] =
    useState("search");
  ...
  const [placeHolderInputSearch, setPlaceHolderInputSearch] = useState(
    "nombre de oficina o abreviación"
  );
  ...
  const submitDateSearch = () => {
    if (!minDate.current || !maxDate.current) {
      addNotification(
        "warning",
        "Aviso",
        "Debe seleccionar ambas fechas para continuar.",
        "top-right",
        8000,
    ...
    if (!fechaInicio.isValid() || !fechaFin.isValid()) {
      addNotification(
        "warning",
        "Error de formato",
        "Las fechas ingresadas no son válidas. Por favor, seleccione una fecha desde el calendario.",
        "top-right",
        8000,
        "fas fa-exclamation-circle",
        null
    ...
    if (fechaInicio.isAfter(fechaFin, "day")) {
      ...
      FuntionSearchData(
        moment(minDate.current).format("YYYY-MM-DD") +
          "," +
          moment(maxDate.current).format("YYYY-MM-DD")
      );
    }
  };
  ...
  const FuntionParameterLevel = (value) => {
    searchDataLevel(value);
  };
    ...
    <>
      ...
      <div className="row">
        <div className="col-lg-2">
          <button
            className="btn btn-theme btn-rounded px-4 rounded-pill"
            onClick={openModal}
          >
            <i className="fas fa-plus fa-1_5x"></i>Nueva oficina
          </button>
        </div>
            ...
            {parameterSearchDataCreate == "data_create" ? (
              <div className="form-group row">
                  ...
                  <div className="row gx-2">
                    ...
                    <div className="col-6">
                      <DateTime
                        ...
                        inputProps={{ placeholder: "Fin", disabled: false }}
                        closeOnSelect={true}
                ...
                <button
                  ...
                  className="btn btn-sm btn-theme m-1 p-0 pt-1 pb-1 btn-rounded rounded-pill"
                >
                  {" "}
              ...
              <>
                <div className="position-relative">
                  <input
                    ...
                    placeholder={"Buscar por " + placeHolderInputSearch + ""}
                    value={searchValue}
                    onChange={(event) => {
                      setSearchValue(event.target.value);
                      FuntionSearchData(event.target.value);
                    }}
                  />
        ...
        <div className="col-lg-3">
          <select
            ...
            onChange={(element) => {
              FuntionSearchParameter(
                element.target.value,
                element.target.options[element.target.selectedIndex].text
              );
              setParameterSearchDataCreate(element.target.value);
            }}
          >
            <option value="search"> nombre de oficina o abreviación</option>
            <option value="parent_search">dependencia</option>
        ...
        <div className="col-lg-2">
          <select
            ...
            value={selectedLevel}
            onChange={(element) => {
              FuntionParameterLevel(element.target.value);
              setSelectedLevel(element.target.value);
            }}
          >
            <option value=""> Todos lo niveles</option>
