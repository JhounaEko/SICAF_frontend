...
const TableList = (getDataRefresch) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            width: '210px',
            selectorKey: 'name',
            cell: (row) => <p>{row.name} <b>({row.initials}) </b> </p>,
        },
        {
            name: (<p className="m-0" style={{ fontWeight: 'bold', fontSize: '15px', textDecoration: 'underline' }}>Dependencia</p>),
            sortable: false,
            selectorKey: 'level',
            cell: (row) => 
                (row.parent) ? (
                    <p style={{ fontSize: '14px' }}>
                        {row.parent.name} <b>({row.parent.initials})</b>
                    </p>
                ) : (
                    <p style={{ fontSize: '14px', color: 'gray' }}>Sin dependencia</p>
                )
        ...
        {
            name: (<p className="m-0 text-center" style={{ fontWeight: 'bold', fontSize: '13px', textDecoration: 'underline' }}>Nivel dependencia</p>),
            sortable: true,
            selectorKey: 'level',
            cell: (row) => <p style={{ fontSize: '14px' }} title="nivel jerargico"> {row.level}  </p>,
            width: '110px',
        },
        {
        ...
        },
        {
            ...
            cell: (row) => (
                <>
                    ...
                    <button className='btn btn-sm btn-theme mx-1' title="Se muestran las ubicaciones de la oficina" onClick={() => onChangeLocation(row.id, row.name)}>
    ...
    useEffect(() => {
        const peticionListOffice = async () => {
            ...
            const returnData = await useListTable(getPag, getSort.column, getSort.order, getCountRows, getDataRefresch.getDataSearh, getDataRefresch.getParameterSearh, getDataRefresch.getLevel);
